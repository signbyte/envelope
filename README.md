...
| `POST /api/v1/envelopes/{id}/slots` | `envelopes:write` | Define a signer slot (order index, role, flow, required LoA, identity ref, and an optional per-signer `returnUrl` overriding the envelope's `origin.returnUrl` — same admission rule); draft-only |
