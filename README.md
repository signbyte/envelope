...
The authenticated API is DPoP-gated (service tokens; the backend-for-frontend acts with the user's delegated token, the signing service with its own). Reads and writes are **owner-filtered** on the caller subject; the signer-side surface additionally admits an invited signer matched on their authenticated eIDAS identity code (the `serial_number` claim — a trusted token claim, never a spoofable header). Both sides of that match are reduced to **one canonical spelling** of the code first — the identity type, the country, a hyphen, and the national code with its separators removed — so however the invitation and the login each write it, one person is one person. A slot's `identityRef` that names no country is refused (**422**, naming the field): this service will not guess a country, because the same digits in another country are another person. A miss returns **404** (absent-or-not-owned is indistinguishable — no resource enumeration), an out-of-scope call **403**, and a stale-version transition **409**.
...
| `POST /api/v1/envelopes` | `envelopes:write` | Create a draft envelope; optional initial document refs (validated on-behalf, hash pinned) + slots, an optional `expiresAt` deadline (RFC 3339 — omitted, the configured default applies), and an optional **`origin`** — the system that prepared the envelope for its own user (`name` required, `returnUrl`, `ref`), so a portal can say *Requested by …* and offer a way back. A return address is admitted only as https, absolute, without credentials or a fragment (`422` otherwise); the caller owns the allowlist it was checked against |
| `GET /api/v1/envelopes` | `envelopes:read` | List the caller's envelopes with an "n of N signed" + "your turn" progress projection and the attached document ids (`docIds`); keyset-paged. **Expired envelopes are excluded from this listing** — once retention has closed an envelope its home is the durable record, not the live listing; the by-id read below still answers for it throughout its keep window. (The data layer takes an `include_expired` flag for a future consumer that needs them listed; this service does not expose it.) With `?documentId=` it becomes a targeted lookup instead: the envelopes covering that one document which the caller may see (owner, or matched participant on a non-draft), newest first, unpaged |
...
| `POST /api/v1/envelopes/{id}/slots` | `envelopes:write` | Define a signer slot (order index, role, flow, required LoA, identity ref — stored canonical, and refused if it names no country — and an optional per-signer `returnUrl` overriding the envelope's `origin.returnUrl` — same admission rule); draft-only |
...
|---|---|---|
| `envelope` | Owner, title, status, order policy, profile, expiry, **version** (the CAS token), and the optional **origin** — the requesting system's registered name, its default return address, its own reference — when a document system prepared the envelope | — |
| `envelope_document` | The document-store id + the **content hash pinned at attach** | Document bytes |
| `signer_slot` | Order index, identity ref, role, flow, required LoA, status, the **signing job id / signature id / signed-document ref** linkage, signer display name, an optional per-signer return address | Signing keys or signature evidence |
